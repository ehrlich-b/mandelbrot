...
void main() {
    ...
    // u_pixelDelta is the complex distance per pixel
    vec2 deltaC = vec2(
        pixelOffset.x * u_pixelDelta.x - pixelOffset.y * u_pixelDelta.y,  // Not exactly right for aspect ratio
        pixelOffset.x * u_pixelDelta.y + pixelOffset.y * u_pixelDelta.x
    );

    // Actually, for simple case: δc = (dx * scale/width, dy * scale/height)
    // Where dx, dy are pixel offsets from center
    vec2 aspectRatio = vec2(u_resolution.x / u_resolution.y, 1.0);
    deltaC = pixelOffset / u_resolution * u_pixelDelta.x * aspectRatio;
